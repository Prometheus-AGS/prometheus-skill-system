...
- [ ] Source keys from ~/.bash_profile (TAVILY_API_KEY, FORGE_MCP_TOKEN); run scripts/codex-provision-mcp-env.sh
- [ ] Install the plugin; run `codex doctor` — confirm the env-var warning clears for the provided keys (or a keyed server initializes)
- [ ] If inherit=all doesn't forward to MCP servers, fall back to per-server inline env + document
- [ ] Record references/env-roundtrip.md; clean up ~/.codex
