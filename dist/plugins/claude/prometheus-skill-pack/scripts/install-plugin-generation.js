...
function stageExecutionComponent(source, staging, expectedRelease) {
...
function stagePluginMetadata(source, staging, expectedRelease) {
  let found = false;
  for (const [platform, manifest] of [
    ...
    if (metadata.version !== expectedRelease) {
      fail(`${platform} plugin version ${metadata.version ?? 'missing'} differs from contract ${expectedRelease}`);
    }
    const destination = path.join(staging, manifest);
    if (!fs.existsSync(destination)) copyEntry(selected, destination);
    found = true;
  }
...
function install(args) {
  ...
  try {
    ...
    stagePluginMetadata(source, staging, contract.releaseVersion);
    stageReleaseRuntimeSupport(source, staging);
    const executionComponent = stageExecutionComponent(source, staging, contract.releaseVersion);
