...
Before each phase and task, emit the canonical `Starting phase …` and
`Starting task …` signals from the active plan. Immediately after each one,
emit its matching `Completed task …` and `Completed phase …` signal. Read the
plan for names and totals; never guess them.
