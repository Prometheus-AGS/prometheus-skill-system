...
# Layout v4 (Prometheus Rules Architecture v4) is opt-in and lives in its own script. Without the flag
# nothing below changes.
_args=("$@")
for ((_i = 0; _i < ${#_args[@]}; _i++)); do
  if [[ "${_args[_i]}" == "--layout" ]]; then
    case "${_args[_i+1]:-}" in
      v4) exec "$SKILL_ROOT/scripts/bootstrap-v4.sh" "$@" ;;
      legacy|"") ;;
      *) printf 'prometheus-context-bootstrap: --layout must be v4 or legacy\n' >&2; exit 1 ;;
    esac
  fi
done
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --layout)  shift 2 ;;   # "legacy" reaches here; "v4" was dispatched above
    -h|--help)
      cat <<'USAGE'
Usage: bootstrap.sh [--path <root>] [--stacks rust,typescript,...]
  ...
  --layout    legacy (default) or v4 — v4 renders CLAUDE.md, path rules and nested
              AGENTS.md from rules/src/ via rules/build.sh; see SKILL.md "Layout v4"
...
source "$SKILL_ROOT/scripts/lib/settings.sh"
wire_settings
