...
Feature: User signs in via the browser
  ...
  Scenario: Happy path — valid credentials land on dashboard
    ...
    And the header greets them by name "alice"
  ...
  Scenario: Empty submission surfaces validation errors
    When they navigate to the sign-in page
    And they submit the form
    Then the email input is marked invalid
    And the password input is marked invalid
