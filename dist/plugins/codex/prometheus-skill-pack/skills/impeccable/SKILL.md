...
1. Run `node <skill-base-dir>/scripts/engine.mjs context` once per session, where `<skill-base-dir>` is the directory that contains this SKILL.md (the skill folder, not a plugin root two levels above it); keep cwd at the user's project. That base directory resolves every `node <skill-base-dir>/scripts/engine.mjs <verb>` command in this skill and its references, and `.agents/skills/impeccable/scripts` is the fallback only when the runtime reports no base directory. On a Windows shell without `sh`, call `node <skill-base-dir>/scripts/engine.mjs` instead. The local Node launcher requires an explicitly configured preinstalled native engine (IMPECCABLE_BIN); it never downloads one. If absent, load existing project context directly and use the permitted guidance.
...
**Pin / Unpin:** `node <skill-base-dir>/scripts/engine.mjs pin <pin|unpin> <command>` creates or removes a standalone `$<command>` shortcut. Report the script's result concisely; relay stderr verbatim on error.
