...
Otherwise run `node <skill-base-dir>/scripts/engine.mjs component-review capture --manifest .impeccable/review/components.json`, then start `node <skill-base-dir>/scripts/engine.mjs component-review serve --session <returned session>` in the background. Open the URL it prints in the available browser and wait for the user; `serve` exits 0 once they submit. Read the result with `node <skill-base-dir>/scripts/engine.mjs component-review verify --manifest .impeccable/review/components.json`; pending, needs-work and stale input all refuse approval. Never submit the page or write a receipt on the user's behalf.
