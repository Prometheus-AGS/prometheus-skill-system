...
```
node <skill-base-dir>/scripts/engine.mjs doctor --json
```
...
- **`auto`** carries no decision. Run `node <skill-base-dir>/scripts/engine.mjs doctor --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
