...
```bash
node <skill-base-dir>/scripts/engine.mjs live-generate --target src/App.jsx --dev-url http://127.0.0.1:5173/ --selector ".pricing-grid" --action bolder --count 3 --boot --wait-for-browser 60000
```
...
- **`event: null`** with `ok: true`: the event was slower than the wait; run `node <skill-base-dir>/scripts/engine.mjs live-poll` once to collect it, then continue.
...
```bash
node <skill-base-dir>/scripts/engine.mjs live-poll --reply EVENT_ID done --file src/App.jsx --then-poll
```
...
- **Interrupted or unsure of the state**: `node <skill-base-dir>/scripts/engine.mjs live-status`, then `live-resume`; the journal under `.impeccable/live/sessions/` is canonical.
