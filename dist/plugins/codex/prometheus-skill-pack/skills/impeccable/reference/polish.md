...
```bash
node <skill-base-dir>/scripts/engine.mjs critique-storage latest "<resolved target>" --json
```
...
```bash
node <skill-base-dir>/scripts/engine.mjs critique-storage close "<resolved target>" "<snapshot_file returned by latest>"
```
