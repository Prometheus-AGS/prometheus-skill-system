#!/usr/bin/env bash
# write-survey-result.sh
# Persist a survey-result.json for a given goal-id.
#
# Usage:
#   write-survey-result.sh --goal-id <id> --result-json '<json>'
#
...
if [[ -z "$RESULT_JSON" ]]; then
  printf '{"ok":false,"error":"--result-json is required"}\n' >&2
  exit 1
fi
...
if ! echo "$RESULT_JSON" | jq empty 2>/dev/null; then
  printf '{"ok":false,"error":"--result-json is not valid JSON"}\n' >&2
  exit 1
fi
...
OUTPUT_PATH="${GOAL_DIR}/survey-result.json"

# Write the result
if ! echo "$RESULT_JSON" | jq . > "$OUTPUT_PATH"; then
  printf '{"ok":false,"error":"Failed to write survey result to %s"}\n' "$OUTPUT_PATH" >&2
  exit 1
fi
