...
- **Layer 4 — hooks:** `.claude/hooks/` (tier guard, single writer, file-lines guard, build guard, sycophancy gate, re-anchor) and `.githooks/commit-msg`. `scripts/check-file-lines.sh` and `scripts/check-architecture.sh` are the same checks on demand.
