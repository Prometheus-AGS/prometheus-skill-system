...
## Deciding: skill, operation, agent, or a combination
...
graph TD
    ...
    Q3 -->|no| PROOF{"Does generated code need<br/>limits, replay, artifacts,<br/>or a signed receipt?"}
    PROOF -->|no| SUB["Use ordinary tools<br/>or a bounded subagent"]
    PROOF -->|yes| EXEC["Prometheus Exec<br/>Tier P or Tier W"]
...
The decision test, stated plainly:

| Signal | Build |
|---|---|
| "I keep re-deriving the same procedure" | **skill** |
| "I need a bounded second opinion inside this session" | **subagent** (`agents/*.md`) |
| "This generated calculation must be constrained, replayable, and independently verifiable" | **Prometheus Exec operation** |
| "This needs to still be running tomorrow, or be callable by another agent" | **native agent** |
| "The agent needs knowledge, a persistent address, and evidenced sub-jobs" | **skill + native agent + explicit Exec adapter** |
...
Prometheus Exec adds a third noun: the **operation** is the bounded, durable unit of work. The installed toolchains or agent create eligible code; Exec runs it and returns evidence. It does not become the agent's model loop or service supervisor. Read the canonical [Dynamic Operations decision guide](/docs/execution/choosing-the-right-capability) before choosing the deployment shape.
