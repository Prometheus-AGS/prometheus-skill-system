...
| FR-016.9 | Tauri mobile: video call uses WebRTC in WebView. Push notifications for incoming calls (via APNS/FCM). Background audio continues when app is minimized. | Should | CallKit integration (iOS) shows incoming call UI. Android uses full-screen notification. |
