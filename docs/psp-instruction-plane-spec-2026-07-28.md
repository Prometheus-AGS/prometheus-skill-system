...
|---|---|---|
| Canonical workflow authority | `substrate/kbd-runtime` persists and replays signed `KbdStateV2` events for lifecycle, plans, active phase/stage/change/task paths, completion dimensions, decisions, blockers, checkpoints, devices, commands, and leases. Compatibility JSON is revision-stamped projection output only. | Implemented and covered by deterministic replay/migration tests. |
| Durable ordering | Sovereign Sync embeds OpenRaft `=0.9.21` with redb log, state machine, membership, snapshot, idempotency, and projection metadata. Lease grants/renewals are committed entries and mutations require revision plus fencing. | The embedded three-voter simulation is green. **Cross-process authenticated iroh Raft transport and the real-device two-machine acceptance run remain release blockers.** |
...
| Public control contract | Versioned command envelopes are shared by CLI, REST, MCP, and SSE. Typed phase/stage/change/task/completion/decision/blocker commands join status, pause, resume, cancel, claim, release, handoff, audit, watch, migrate, and rollout gates. | Implemented. Remote mutations do not have an independent compatibility-file fallback. |
| Harness convergence | One capability manifest generates Claude, Codex, OpenCode, and Kimi adapters. Emergency pause is checked before runtime access; writer roles require a native pre-mutation fence guard. Prompt/Stop chains defer noncritical memory, summary, learning, and proposal work. | Claude and OpenCode active integration is covered locally. Generated Codex and Kimi artifacts still need real installed-host acceptance before release. |
...
1. Cross-process authenticated Raft transport over paired devices, including two-device/three-voter partition, takeover, reconnect, and stale-fence rejection tests.
...
The Cross-Harness Control Plane plan (Codex harness, in progress) owns the **state/control plane**: event-sourced KBD runtime, lifecycle states, lease/fencing, Sovereign Sync KBD domain, schema unification, CI consolidation, harness payload parity, and Stop-hook redesign. Audit of the working tree confirms its P0 operator-safety patch has substantially landed — `position-stop-gate.sh` now checks `stop_hook_active`, honors `.kbd-orchestrator/PAUSE` before any parsing, keys deduplication to session + state-revision fingerprint (explicitly excluding transcript size), recognizes suspended states, and is advisory-only. `waypoint-render.sh` implements suspended/terminal vocabularies and dual-casing reads. The mtime freshness check on `position.json` remains, consistent with P1 not yet landed.

**Verdict on that plan:** sound diagnosis, correct P0 priorities, correct architectural direction (event journal + single projection writer + leased handoff, matching Temporal/LangGraph/OpenHands precedent). Three flags, none blocking:
...
3. **Loro is doing less than it appears.** With a single-writer lease, the journal needs replication, not CRDT merge semantics. Using Loro as transport plus ephemeral awareness for presence is defensible (it is already in Sovereign Sync), but do not let CRDT merge paths become load-bearing for the journal — that would reintroduce the multi-writer ambiguity the lease exists to eliminate. Confirm the runtime library language is Rust; the plan does not state it.
...
**1.2 — 2026-07-28.** Recorded the production-convergence implementation rather than preserving the document as a stale proposal. Added §0 with the canonical KbdStateV2/OpenRaft/redb/security/control-interface implementation, native lifecycle adapters, bounded reanchor, 145-skill structural lint and 14-target parity, 36-prompt deterministic grader, identity-safe migration, shadow/canary gates, and explicit release blockers. Corrected the prior sentinel-first compaction design: native host lifecycle events are authoritative when available. Deferred the full 145-skill scoring/rewrite and rationalization program until live activation and budget evidence exists. No discovery ceiling or production-readiness claim was inferred.
