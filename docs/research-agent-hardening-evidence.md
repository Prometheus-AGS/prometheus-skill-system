...
### Repair status — updated 2026-09-08 by change-drt-007

The successor phase's `change-drt-007-install-surface-repair` addressed both.

| Id | Status | What changed |
|---|---|---|
| **D-A** | **REPAIRED at the template** | `substrate/prometheus-research/com.prometheus.research.plist` now declares `EnvironmentVariables` with `HOME`/`USER`/`PATH` placeholders, matching the `shared/launchagents/*.plist` convention it previously ignored; `scripts/install-binaries.sh` substitutes all three and **fails loudly** if any placeholder survives rendering. Proven by `skills/research/deep-research/tests/installed-service-smoke.sh`: the rendered plist is valid, its PATH is non-empty and resolves `claude`, and a negative control confirms the old `__HOME__`-only rendering leaves PATH unusable. Proven **end-to-end under launchd**, not only at the template: `SMOKE_LAUNCHD=1` bootstraps the rendered plist as a throwaway label and starts a real job, which reports a non-null `harness_pid` (`job-1788909316-22a2fcf7`, 2026-09-08) instead of `blocked: no harness binary on PATH` |
| **D-B** | **VISIBLE, not yet repaired** | The daemon now runs an execution self-check at startup and exposes it at `/health`, reporting the resolved harness and driver with paths, sizes, and — for a stale driver — exactly which stage-contract markers are missing. A pre-contract stub is reported `stale` **before a job is started** instead of after one fails. The installed driver itself is **still the 1920-byte stub**: the republish is blocked by 14 foreign `artifact-refiner` installs that this phase does not own (see the change spec, task 4). `RESEARCH_DRIVER` remains the working override |

Neither defect was in the daemon's own logic. In both cases it refused to report
a success it had not achieved, which is what made them findable at all.
