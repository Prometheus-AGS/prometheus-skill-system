...
not the source, and under the conditions that reproduced the original failure. Installed
surface discovery SHALL match production installer discovery and MUST NOT classify skills
below `tests` or `fixtures` directories as product payloads.
...
#### Scenario: Test fixtures contain skill manifests
- **WHEN** a source skill contains nested `SKILL.md` files below `tests` or `fixtures`
- **THEN** installed-surface verification SHALL exclude those manifests exactly as the
  production installers do
