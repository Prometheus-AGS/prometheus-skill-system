...
- [ ] 1.2 Implement execution job/port abstractions, deterministic policy outcomes, and receipt assembly in `exec-core`
- [ ] 1.3 Implement atomic artifact CAS storage, safe output collection, pins, and budgeted garbage collection
...
- [ ] 3.1 Implement the durable request/run ledger, same-hash replay, hash conflict, terminal commit ordering, and restart reconciliation
- [ ] 3.2 Implement ordered durable events and response-loss retrieval in the transport-independent service
- [ ] 3.3 Implement health-first mode-0600 UDS binding, same-UID peer credentials, readiness state, and REST/SSE routes
- [ ] 3.4 Extend `prometheus-exec` with daemon, run, status, and non-mutating doctor commands

## 4. Local certification and handoff

- [ ] 4.1 Run local format, warnings-denied Clippy, unit/property/integration, sandbox, restart, API, and false-green doctor tests
