...
execFileSync(
  process.execPath,
  [path.join(root, 'scripts/generate-skill-system-distribution.js'), ...(check ? ['--check'] : [])],
  { cwd: root, stdio: 'inherit' }
);

const contract = JSON.parse(fs.readFileSync(path.join(root, 'skill-system.json')));
const manifest = JSON.parse(
  fs.readFileSync(path.join(root, contract.outputs.codexPackage, '.codex-plugin/plugin.json'))
);
const marketplace = JSON.parse(fs.readFileSync(path.join(root, contract.outputs.codexMarketplace)));

if (manifest.version !== contract.releaseVersion)
  throw new Error('Codex package version differs from the distribution contract');
if (manifest.skills !== './skills')
  throw new Error('Codex package must expose the flattened skills directory');
if (manifest.hooks !== undefined)
  throw new Error('Codex package contains the rejected hooks field');
