---
id: index
title: Substrate Crates
---

# Substrate Crates

Rust crates powering the learn domain and research infrastructure. Each page
is a summary that links to the canonical crate source — the code and its doc
comments are the source of truth.

- [storage-provider](/docs/substrate/storage-provider) — persistence traits + privacy classes
- [learner-model](/docs/substrate/learner-model) — CRDT learner state + FSRS-6
- [surface-bridge](/docs/substrate/surface-bridge) — Tier 2 UI server (:7890)
- [sovereign-client](/docs/substrate/sovereign-client) — Rust SDK for sovereign-sync
- [prometheus-research](/docs/substrate/prometheus-research) — deep-research daemon (:7891)
- [sovereign-sync](/docs/sovereign-sync/overview) — P2P CRDT sync daemon (existing section)
