...
| Step 8 (learner-model observation POST) | Calls `${LEARNER_MODEL_URL}/api/v1/observations` to update the CRDT learner model | **Skipped entirely.** No real `learner_id` exists in an eval context; posting synthetic eval grades into the learner-model would pollute it with fake observations. | Eval items are not real learners; writing to the learner-model substrate would be a side effect with no correct undo. |
