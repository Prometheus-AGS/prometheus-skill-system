...
4. **Execute** (`prompts/execute.md`) — Apply changes using domain-appropriate tools
   - For software domain: delegates to **kbd-process-orchestrator** inner loop
   - KBD decomposes plan items into fine-grained changes
   - KBD auto-detects OpenSpec for structured change management
   - KBD runs artifact-refiner QA per completed change
   - See `references/integrations/kbd-orchestrator.md` for the delegation protocol
5. **Reflect** (`prompts/reflect.md`) — Measure movement, compare before/after, decide next steps
   - Aggregates KBD phase results via evolver-bridge.json
   - Includes artifact quality metrics from artifact-refiner
...
### Nested Loop Architecture (Software Domain)

```
EVOLVER (strategic cycle)
  Assess → Analyze → Plan ──────────────────────┐
                                                  │
  Execute ────────────────────────────────────────┤
    └─ KBD INNER LOOP (tactical cycle)           │
        ├─ kbd-plan: decompose items → changes   │
        │   └─ OpenSpec detection (auto)          │
        ├─ kbd-execute: multi-tool dispatch       │
        │   └─ artifact-refiner QA per change     │
        └─ kbd-reflect: aggregate → evolver       │
                                                  │
  Reflect ← results from KBD ────────────────────┘
