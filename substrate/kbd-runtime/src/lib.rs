...
impl Runtime {
    ...
    fn write_compatibility_projections_from_state_inner(
        ...
        // position-reminder.txt — the projection agents actually read every turn.
        //
        // Until 2026-08-12 nothing wrote this file: it was hand-authored, so it
        // drifted the moment a phase changed. On that date it sat four days
        // stale, naming a phase that was already `complete`, and a second
        // harness read it, concluded it was in that phase, and stalled for
        // three cycles against work that belonged to a different one.
        //
        // Regenerating it here — from the same `state` that produced the
        // waypoint above — makes the two physically incapable of disagreeing.
        //
        // Note the asymmetry with the `progress.json` loop below, which REFUSES
        // to clobber files it did not author. That guard exists because
        // progress.json carries agent-authored content. This file does not: it
        ...
        let reminder = format!(
            "POSITION REMINDER — read this as your FIRST tool call every turn\n\
             ...
             transition and any manual change is discarded. To move position,\n\
             use `prometheus kbd phase activate|transition`.\n\
             \n\
            ...
            next = state
                .exact_next_work
                .as_deref()
                .unwrap_or("(none recorded — set via `phase activate --exact-next-work`)"),
            rev = state.revision,
            ts = projection_time.to_rfc3339(),
        );
        atomic_text(&kbd_root.join("position-reminder.txt"), &reminder)?;
...
mod tests {
    ...
    /// drifted four days behind canonical state, named an already-`complete`
    /// phase, and caused a second harness to stall for three cycles. The test
    /// asserts the file is (a) produced by a projection and (b) carries the
    /// phase canonical state actually holds — not whatever was there before.
    #[test]
    fn position_reminder_is_regenerated_from_canonical_state() {
        let dir = tempdir().unwrap();
        let kbd = dir.path().join(".kbd-orchestrator");
        let phase = kbd.join("phases/reminder-phase");
        fs::create_dir_all(&phase).unwrap();
        fs::write(
            phase.join("progress.json"),
            r#"{"phase":"reminder-phase","changes":[],"ordered_changes":[{"id":"C-001","title":"only change"}]}"#,
        )
        .unwrap();

        // A stale reminder naming a phase that is NOT the one canonical state
        // will hold — exactly the shape of the incident.
        fs::write(
            kbd.join("position-reminder.txt"),
            "Phase: some-old-closed-phase\nStep: 1 of 21\n",
        )
        .unwrap();
        ...
        assert!(
            !reminder.contains("some-old-closed-phase"),
            "stale reminder survived a projection: {reminder}"
        );
        assert!(
            reminder.contains("reminder-phase"),
            "reminder does not name the canonical phase: {reminder}"
        );
        assert!(
            reminder.contains("kbd-runtime"),
            "reminder must declare its generator so it is not hand-edited: {reminder}"
        );
        assert!(
            reminder.contains("Do NOT hand-edit"),
            "reminder must warn against hand edits: {reminder}"
        );

        // The counter is a PROJECT-WIDE roll-up — `completion` is held on
        // runtime state, not per phase. The first cut of this projection
        // labelled it "THIS phase's implementation counter", which is exactly
        // the misreading the file is supposed to prevent: it printed 47 of 72
        // for a phase that was 0 of 6. Assert the label matches the source.
        assert!(
            reminder.contains("PROJECT-WIDE") && reminder.contains("ALL phases"),
            "the counter must be labelled project-wide, not phase-scoped: {reminder}"
        );
        assert!(
            !reminder.contains("THIS phase's implementation counter"),
            "reminder must not claim the roll-up is phase-scoped: {reminder}"
        );
    }
