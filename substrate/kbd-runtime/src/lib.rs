    ...
    fn write_compatibility_projections_from_state_inner(
        ...
        let reminder = format!(
            "POSITION REMINDER — read this as your FIRST tool call every turn\n\
             Phase: {phase}\n\
             Stage: {status}\n\
             Progress: {done} of {total} (implementation)\n\
             Next command: {next}\n\
             \n\
             ...
             \n\
             Progress above is THIS phase's implementation counter. The\n\
             project-wide roll-up lives in current-waypoint.json; do not read one\n\
             for the other.\n",
            phase = waypoint_phase,
            status = lifecycle_name(&state.lifecycle),
